...
|-------------------------------------|
|  | 
As aproximações de Laplace baseiam-se num princípio semelhante às coordenadas generalizadas de movimento descritas no quadro 4.2. A ideia é que a energia livre pode ser aproximada por uma expansão quadrática em torno do modo posterior ($\mu$). Em uma dimensão, é o seguinte: 

$$ \begin{equation} F[y,q]= E_{q(x)}[\ln q(x) - \ln p(y,x) \end{equation}$$

$$ \begin{aligned}  \approx E_{q(x)} [ \ln q(\mu) + 
\begin{matrix}
